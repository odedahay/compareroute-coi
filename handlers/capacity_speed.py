...
class capacitySpeedHandler(webapp2.RequestHandler):
    def post(self):
        ...
        try:    
            ...
            if  not error:
                ...
                if custPostal_arr_sorted is None:
                    ...
                    results = []
                    temp_result = []
                    for postal_order in custPostal_arr_sorted:
                        temp_result.extend(postal_dict[int(postal_order)])
  
                    for vehicle in range(1,num_of_vehicle+1):
                        orders = []
                        temp_capacity = 0
                        for i in temp_result:
                            if order_taken_dict[i] == True:
                                if temp_capacity + capacity_dict[i] <= vehicle_capacity:
                                    orders.append(i)
                                    
                                    temp_capacity+= capacity_dict[i]
                                    order_taken_dict[i] = False
                        results.append(orders)        
                    remaining_orders = []
                    for item in temp_result:
                        
                        if order_taken_dict[item] == True:
                    ...
                    for result in results:
                        ...
                        if len(remaining_orders) != 0:
                            json_vehicle_dict["remaining orders"] = remaining_orders 
            if error: 
...
app = webapp2.WSGIApplication([
    ('/json/capacity_speed',capacitySpeedHandler)
], debug=True)
